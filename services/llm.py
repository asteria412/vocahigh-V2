...
# =============================================================================
# [SECTION 1] 파싱된 단어 검증 및 빈칸 보정부 (배치 처리)
# =============================================================================

BATCH_SYSTEM_PROMPT = """
당신은 '단어장 무결성 검수자'입니다. 여러 한자 항목을 한 번에 검수합니다.

[임무]
1. 각 항목의 한자가 원본 조각 내에서 단어-병음-뜻 구조를 가진 '진짜 단어'인지 확인하세요.
2. 진짜라면 원본에 적힌 병음과 뜻을 정확히 추출하세요.
3. 단어가 아닌 텍스트(예문 파편, 섹션 제목, 페이지 번호 등)라면 is_noise: true로 표시하세요.

반드시 JSON으로 응답: {"results": [{"zh": "한자", "pinyin": "병음", "ko": "뜻", "is_noise": false}, ...]}
입력 순서와 동일하게, 입력한 항목 수만큼 반환하세요.
"""

def process_vocab_with_llm(df, raw_text, batch_size=10):
    """
    df: 1차 파싱된 데이터
    raw_text: PDF 전체 원본
    batch_size: API 호출 1번당 처리할 항목 수 (기본 10)
    """
    ...
    for batch_start in range(0, len(items), batch_size):
        ...
        for idx, row in batch:
            char_pos = raw_text.find(row['zh'])
            if char_pos == -1:
                indices_to_drop.append(idx)
                continue
            snippet = raw_text[max(0, char_pos - 100):min(len(raw_text), char_pos + 400)]
            batch_input.append({"zh": row['zh'], "snippet": snippet})
        ...
        try:
            ...
            for i, res in enumerate(results):
                ...
                if res.get('is_noise') is True:
                    ...
                    df.at[idx, 'pinyin'] = res.get('pinyin', df.at[idx, 'pinyin'])
                    df.at[idx, 'ko']     = res.get('ko',     df.at[idx, 'ko'])
                    df.at[idx, 'flags']  = 'OK'
        except:
            pass
